...
beethowen_ns = cg.esphome_ns.namespace("beethowen")
BeethowenHub = beethowen_ns.class_("BeethowenHub", cg.Component)

DumpOption = beethowen_ns.enum("DumpOption")
DUMP_OPTION = {
...
RoleOption = beethowen_ns.enum("RoleOption")
ROLE_OPTION = {
    "NONE": RoleOption.RoleOption_None,
    "DESTINATION": RoleOption.RoleOption_Destination,
    "SOURCE": RoleOption.RoleOption_Source,
    "BOTH": RoleOption.DumpOption_Both
}

CONFIG_SCHEMA = cv.All(
        cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(BeethowenHub),
            cv.Optional(CONF_DUMP_OPTION): cv.enum(
                DUMP_OPTION, upper=True, space="_"
            ),
            cv.Optional(CONF_ROLE_OPTION): cv.enum(
                ROLE_OPTION, upper=True, space="_"
            ),
        }
    )
...
def beethowen_shared_sensor_configs(is_binary_sensor, MEASUREMENT_TYPES):
  ...
  cpp_classname = "BeethowenBinarySensor" if is_binary_sensor else "BeethowenSensor"

  BeethowenDevice = beethowen_ns.class_("BeethowenDevice", cg.Component)
  ...
  BeethowenSensor = beethowen_ns.class_(cpp_classname, sensor_base, cg.Component)

  CONFIG_SCHEMA = cv.All(
    cv.Schema(
      {
        cv.GenerateID(CONF_Beethowen_ID): cv.use_id(BeethowenHub),
        cv.GenerateID(): cv.declare_id(BeethowenDevice),
        ...
        cv.Required(CONF_SENSORS): cv.All( 
          cv.ensure_list(
            schema_base.extend(
              {
                  cv.GenerateID(): cv.declare_id(BeethowenSensor),
